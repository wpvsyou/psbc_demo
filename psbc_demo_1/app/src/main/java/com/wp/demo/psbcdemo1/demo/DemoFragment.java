...
public class DemoFragment extends BaseFragment implements OnClickListener {
	...
	protected void checkLogin() {
		if (null == mUsername.getText()
			...
			String selection = PSBCDatabaseHelper.Personnel.USER_NAME + "=?";
			String[] selectionArgs = new String[] { mUsername.getText() };
			Cursor cursor = getActivity().getContentResolver().query(
					PSBCContentProvider.PERSONNEL_URI, null, selection,
					selectionArgs, null);
			if (null != cursor && cursor.moveToFirst()) {
				try {
					String locPassword = cursor
							.getString(cursor
									.getColumnIndex(PSBCDatabaseHelper.Personnel.PASSWORD));
					if (TextUtils.equals(locPassword, mPassword.getText())) {
						String token = cursor
								.getString(cursor
										.getColumnIndex(PSBCDatabaseHelper.Personnel.ID));
