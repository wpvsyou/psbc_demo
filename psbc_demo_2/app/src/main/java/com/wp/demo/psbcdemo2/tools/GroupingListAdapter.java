...
public abstract class GroupingListAdapter extends BaseAdapter {
    ...
    private int mRowIdColumnIndex; //行ID列索引

    /**
     * Count of groups in the list.
     */
    private int mGroupCount; //组计数

    /**
     * Information about where these groups are located in the list, how large they are
     * and whether they are expanded.
     */
    private long[] mGroupMetadata;//源数据

    private SparseIntArray mPositionCache = new SparseIntArray();//位置高速缓存
    private int mLastCachedListPosition;//最后缓存List的位置
    private int mLastCachedCursorPosition;//最后缓存Cursor的位置
    private int mLastCachedGroup;//组最后缓存
    ...
    private int idealByteArraySize(int need) {
        for (int i = 4; i < 32; i++)
            if (need <= (1 << i) - 12)//1064  2036/8  = 254/8 =31
                return (1 << i) - 12;
    ...
    private void sendPosition(int arg0)
    {
        mPosition = arg0;
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (view == null) {
            ...
            if (mPositionMetadata.itemType == ITEM_TYPE_STANDALONE) {
                view = newStandAloneView(mContext, parent);

            } else if (mPositionMetadata.itemType == ITEM_TYPE_GROUP_HEADER) {
                view = newGroupView(mContext, parent);

            } else if (mPositionMetadata.itemType == ITEM_TYPE_IN_GROUP) {
